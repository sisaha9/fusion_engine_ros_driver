...
FusionEngineSerialInterface::FusionEngineSerialInterface(
    const std::shared_ptr<point_one::fusion_engine::parsers::FusionEngineFramer>
        framer,
    const rclcpp::Logger& logger, std::string& device_name,
    const std::string& flow_control, const std::string& parity,
    const std::string& stop_bits, const uint32_t baud_rate)
    : logger_(logger) {
  framer_ = framer;
  InitDeviceConfig(flow_control, parity, stop_bits, baud_rate);
  owned_ctx_ = std::make_unique<drivers::common::IoContext>(NUM_THREADS);
  serial_driver_ =
      std::make_unique<drivers::serial_driver::SerialDriver>(*owned_ctx_);
...
void FusionEngineSerialInterface::InitDeviceConfig(
    const std::string& flow_control, const std::string& parity,
    const std::string& stop_bits, const uint32_t baud_rate) {
  ...
  try {
    ...
    if (!serial_driver_->port()->is_open()) {
      ...
      serial_driver_->port()->async_receive(
          std::bind(&FusionEngineSerialInterface::SerialCallback, this,
                    std::placeholders::_1, std::placeholders::_2));
    }
  } catch (const std::exception& e) {
    RCLCPP_ERROR(logger_, "Error creating serial port: %s - %s",
                 device_name_.c_str(), e.what());
  }
  return true;
}

void FusionEngineSerialInterface::SerialCallback(
