...
: rclcpp::Node("fusion_engine_ros_driver_node", options)
{
  ...
  udp_subscriber_ = this->create_subscription<udp_msgs::msg::UdpPacket>(
    ...
    std::bind(&FusionEngineRosDriverNode::UdpCallback, this, std::placeholders::_1));
  serial_subscriber_ = this->create_subscription<std_msgs::msg::UInt8MultiArray>(
    "fusion_engine_serial_packets", rclcpp::QoS{100},
    std::bind(&FusionEngineRosDriverNode::SerialCallback, this, std::placeholders::_1));

  pose_publisher_ =
    this->create_publisher<geometry_msgs::msg::PoseStamped>("pose", rclcpp::SensorDataQoS());
  gps_fix_publisher_ =
    this->create_publisher<gps_msgs::msg::GPSFix>("gps_fix", rclcpp::SensorDataQoS());
  nav_sat_fix_publisher_ =
    this->create_publisher<sensor_msgs::msg::NavSatFix>("nav_sat_fix", rclcpp::SensorDataQoS());
  ...
  framer_->SetMessageCallback(std::bind(
    &FusionEngineRosDriverNode::PublishMessage, this, std::placeholders::_1,
    std::placeholders::_2));
}
  ...
  const point_one::fusion_engine::messages::MessageHeader & header, const void * payload_in)
{
  ...
  if (header.message_type == point_one::fusion_engine::messages::MessageType::ROS_GPS_FIX) {
    auto & contents =
      *reinterpret_cast<const point_one::fusion_engine::messages::ros::GPSFixMessage *>(payload);
    ...
    auto & contents =
      *reinterpret_cast<const point_one::fusion_engine::messages::ros::IMUMessage *>(payload);
    ...
    auto & contents =
      *reinterpret_cast<const point_one::fusion_engine::messages::ros::PoseMessage *>(payload);
