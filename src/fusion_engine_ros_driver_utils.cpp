  ...
  gps_msgs::msg::GPSFix::SharedPtr & gps_fix_msg)
{
  ...
  gps_fix_msg->time =
    contents.p1_time.seconds + (contents.p1_time.fraction_ns * 1e-9);  // time since power-on
  ...
  std::copy(
    ...
    std::begin(gps_fix_msg->position_covariance));
  ...
  sensor_msgs::msg::NavSatFix::SharedPtr & nav_sat_fix_msg)
{
  ...
  std::copy(
    std::begin(contents.position_covariance_m2), std::end(contents.position_covariance_m2),
    std::begin(nav_sat_fix_msg->position_covariance));
  ...
  sensor_msgs::msg::Imu::SharedPtr & imu_msg)
{
  ...
  std::copy(
    ...
    std::begin(imu_msg->orientation_covariance));
  ...
  std::copy(
    std::begin(contents.angular_velocity_rps), std::end(contents.angular_velocity_rps),
...
{
  if (!fusion_engine_server_data.empty()) {
    const uint8_t * buffer = fusion_engine_server_data.data();
    framer->OnData(buffer, sizeof(uint8_t) * fusion_engine_server_data.size());
  }
}
}  // namespace ros_utils
}  // namespace fusion_engine
