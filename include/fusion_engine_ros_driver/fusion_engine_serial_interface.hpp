...
class FusionEngineSerialInterface {
 public:
  explicit FusionEngineSerialInterface(
      const std::shared_ptr<
          point_one::fusion_engine::parsers::FusionEngineFramer>
          framer,
 ...
 private:
  ...
  void InitDeviceConfig(const std::string& flow_control,
                        const std::string& parity, const std::string& stop_bits,
                        const uint32_t baud_rate);
  void SerialCallback(const std::vector<uint8_t>& buffer,
